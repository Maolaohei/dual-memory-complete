...
**Dual-Memory Complete** 是一个生产级的 AI Agent 记忆系统，v5.0 带来革命性升级：
...
- **🧠 HyDE 智能检索** - 语义召回率大幅提升
- **⏰ 时间衰减评分** - 新鲜记忆优先展示
- **💾 Token 预算管理** - 2600 token 硬限制
- **🔄 会话自启动** - 新会话自动初始化 🆕
- **📁 文件变更监听** - MD 变更自动同步 🆕
- **🎯 任务优化系统** - 同类任务节省 3200+ token 🆕
...
|------|------|------|------|
| **新会话启动** | 手动/不稳定 | 全自动 | ✅ |
| **模型冷启动** | 4s 阻塞对话 | 后台预热，无感知 | ✅ |
| **MD变更同步** | 每次检索检查 | 自动增量同步 | ✅ |
| **固定 Token** | 全量加载 | 按需，减少 60%+ | ✅ |
| **同类任务** | 每次重新探索 | 节省 3200+ token/次 | ✅ |
| **记忆重复率** | 无控制 | 去重后接近 0 | ✅ |
| **Token 预算** | 软限制 | 硬限制 2600 | ✅ |
